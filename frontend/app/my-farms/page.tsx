          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {farms.map((farm) => (
              <div 
                ...
                className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 cursor-pointer hover:border-[#0782c5] hover:shadow-lg transition-all group"
              >
                <div className="flex justify-between items-start mb-4">
                  <h3 className="text-2xl font-black text-slate-800 group-hover:text-[#0782c5] transition-colors">
                    {farm.farm_name}
                  </h3>
                  {farm.is_default && (
                ...
                <div className="inline-block px-3 py-1 bg-slate-100 text-slate-600 text-xs rounded-full font-bold">
                  {farm.soil_condition} Soil Drainage
                </div>
              </div>
            ))}
          </div>
